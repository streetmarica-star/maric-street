...
Coleta de Dados: O morador registra a ocorrência através do Google Forms.

Armazenamento: Os dados do formulário são salvos automaticamente em uma Planilha Google (Google Sheets), que atua como banco de dados em tempo real.

Leitura dos Dados: A aplicação web utiliza a biblioteca PapaParse.js para carregar o arquivo CSV da planilha publicada.

Geocodificação: Um script JavaScript utiliza a API do Nominatim (OpenStreetMap) para converter o endereço de cada ocorrência (Logradouro da Ocorrência) em coordenadas geográficas.

Exibição no Mapa: A biblioteca Leaflet.js renderiza o mapa e adiciona marcadores para cada ocorrência mapeada.
...
Estrutura, Estilização e Lógica de Processamento.

Mapeamento

Leaflet.js

Biblioteca principal para renderização do mapa interativo.

Geocodificação

Nominatim (OpenStreetMap)

Serviço para conversão de endereços em coordenadas geográficas.

Dados

PapaParse.js

Parser de CSV para leitura rápida dos dados da Planilha Google.

Plataforma

Google Forms, Google Sheets, GitHub Pages

Coleta de dados, Armazenamento e Hospedagem.
...
Instituto Federal Fluminense (IFF), Campus Maricá
...
Linguagem de Programação de Aplicações Móveis
...
Pós-graduando em Gestão de Projetos e Negócios em Tecnologia da Informação (IFRJ).

Pesquisador no Laboratório de Projetos de Tecnologia da Informação, Negócios e Economia Criativa (LABPROJTNE) do IFRJ.

Suas áreas de pesquisa e estudo incluem Tecnologia da Informação (T.I), Inteligência Artificial (I.A), Educação em T.I, Blockchain e Cidades Inteligentes.
...
🗺️ Aplicação (Mapa): https://streetmarica-star.github.io/maric-street/

📝 Formulário de Registro: https://docs.google.com/forms/d/e/1FAIpQLSdSXiy54n82-7ZMxWdNZ_cX2HrLLAcx6_fSdAum562tBG7mzA/viewform
